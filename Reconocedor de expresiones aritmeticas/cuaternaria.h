#include <stdio.h>
#include <stdlib.h>
#include <string.h> 
#include <math.h>
#include "funciones.h"
#define longitud 256
#define baseCuaternaria 4 

struct NumeroCuaternario
{
    long double representacionNumerica;
    char representacionSimbolica[longitud];
};

struct NumeroCuaternario cuaternario;

/*!
    Aquí se normaliza el string desde consola,
    es decir, se retiran los signos (-,+) además
    de las letras (c, C). 
*/
char *prepararEntradaC(char expresion[longitud])
{
    int j = 0;
    char *tempCadena = malloc(longitud);

    for (int i = 0; i < strlen(expresion); i++)
    {
        switch (expresion[i])
        {
        case '-': break;
        case '+': break;
        case 'c': break;
        case 'C': break;
        default:
            tempCadena[j] = expresion[i];
            j++;
        }
    }
    tempCadena[j] = '\0';
    return tempCadena;
}

/*!
    Esta función transforma el string ingresado desde
    consola a un número en base 10 para poder realizar 
    operaciones con él. 
 */
void cuaToDecDouble(char expresion[longitud])
{
    int signoDelNumero;             //? Puede ser negativo o positivo

    signoDelNumero = asignarSigno(expresion);
    char* representacionSimbolica = prepararEntradaC(expresion);

    strcpy(cuaternario.representacionSimbolica, expresion);
    cuaternario.representacionNumerica = representarNumericamente(representacionSimbolica, signoDelNumero, baseCuaternaria);

    mostrarResultadoEnTodasLasBases(cuaternario.representacionNumerica);

}